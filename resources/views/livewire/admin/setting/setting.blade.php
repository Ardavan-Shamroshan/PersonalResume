    ...
    <div class="content-wrapper">
        ...
        <div class="row">
            <div class="col-lg-12 grid-margin stretch-card">
                <div class="card">
                    <div class="card-body">
                        <div><small class="text-muted py-3">Showing {{ $setting->count() }} entries.</div>
                                ...
                                <tbody>
                                    <tr>
                                        @if ($editedSettingIndex == $setting->id)
                                            <td style="width: 8rem;">
                                                <input type="file" class="form-control" id="logo" placeholder="logo" name="settingLogo" wire:model.lazy="settingLogo">
                                            </td>
                                            <td>
                                            ...
                                            <td><input type="text" class="form-control @error('setting.copy_right') border-danger text-danger @else border-success text-success @enderror" wire:model.lazy="setting.copy_right"></td>
                                            <td>
                                                <a class="btn btn-sm btn-icon-text my-auto @if ($errors->any()) disabled-link @endif" wire:click="submit"> <small>Save <i class="mdi mdi-content-save"></i></small></a>
                                            </td>
                                        @else
                                            <td style="width: 19%;">
                                                @if (!$settingLogo && $setting->logo)
                                                    <img src="{{ asset($setting->logo) }}" class="rounded border p-1 w-100" alt="preview.png" style="height:100px">
                                                @elseif($settingLogo)
